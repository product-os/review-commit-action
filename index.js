...
class ApprovalAction {
  ...
  async findCommitComment(commitSha) {
    ...
    const comment = comments.find(
      (comment) =>
        ...
        comment.user.login === this.octokit.user.login
    );
  ...
  async createReaction(commentId, content) {
    const { data: reaction } = await this.octokit.rest.reactions.createForCommitComment({
      ...this.context.repo,
  ...
  async waitForApproval(commentId) {
    ...
    this.waitReactionId = (await this.createReaction(commentId, this.waitReaction)).id;

    while (true) {
      const reactions = await this.getReactions(commentId);

      const rejectedBy = reactions.find(
        (r) => r.content === this.rejectReaction
      )?.user.login;
      ...
      const approvedBy = reactions.find(
        (r) => r.content === this.approveReaction
      )?.user.login;
